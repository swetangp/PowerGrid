...
//Singleton Pattern, uses this function to increment the global variable
void incrementLoop(void)
{
	Singleton::instance()->increaseValue();
}
//Function that returns a global variable from the Singleton pattern
int returnLcount()
{
	return Singleton::instance()->getLvalue();
}
//Function that resets the global variable back to zero in the Singleton pattern class
...
int main()
{
	...
	*/
	...
	//The design patterns below each use a predefined scenario to demonstrate each respective parts of the assignment
	//Due to the length of the board game we will demonstrate a certain phase in this game displaying the observer design pattern and singleton pattern
	cout << "3.Observer Design Pattern parts 1 and 2" << endl;
	cout << "4. Singleton Pattern part 4" << endl;
	int phaseDecision;
	cin >> phaseDecision;
	...
	if (phaseDecision == 1)
	{
		//First we want the users to see the whole map and determine how many players they want to play with
		PowerGrid.printMap();
		cout << "How many players will be player this game?(2 minimum, 6 maximum)" << endl;
		int numberOfPlayers;
		cin >> numberOfPlayers;

		//As of right now only 2 players can be selected due to issues I had with shuffling the players position.
		while (numberOfPlayers < 1 || numberOfPlayers>6)
		{	
			cout << "Please enter a valid range for the number of players. (2-6 players)" << endl;
			cin >> numberOfPlayers;
		}
		//The player is provided the summary card, houses, and 50 elektro
		vector<Player> p;
		...
		//The first player will now select an area in which they will begin in.
		...
		//The second player will now select an area. They must choose an area in which that it is adjacent to the previous players area.
		...
		//We have 3 deck of cards, the Powerplant Card deck, the present market deck and the future market deck.
		Card ppc;
		...
		//The cards chosen here are for demonstration purposes
		...
		//the PowerPlant card deck will now erase whichever copies that are now in the present and future market decks.
		...
		while (mainGameLoop == 0)
		{
			...
			while (exit == 9)
			{
				...
				if (p[i].getCurrentTurn() < p[j].getCurrentTurn())
				{
					...
					g.display();
					g.turnInformationDisplay(phaseDecision, PowerGrid, p[i], actualMarketCard);
				...
				else
				{
					...
					while (bid == 0)
					{
						...
						g.Update();
						g.turnInformationDisplay(phaseDecision, PowerGrid, p[j], actualMarketCard);
	...
	if (phaseDecision == 3)
	{
		...
		//p[0].setResourceZero();p[1].setResourceZero();p[2].setResourceZero();	
		...
		while (loop == 0)
		{
			if (p[returnLcount()].getCurrentTurn() == reverseOrder)
			{
				...
				GameView g(PowerGrid, p[returnLcount()]);
				g.Update();
				g.turnInformationDisplay(phaseDecision, PowerGrid, p[returnLcount()], actualMarketCard);
		...
		//PART 2 BELOW
		//Implemented Observer design for view of different statistics
		for (int i = 0; i < p.size(); i++)
		{
			GameView myView(PowerGrid, p[returnLcount()]);
			myView.usefulInformation();
			incrementLoop();
		}
		system("pause");
		return 1;
	}
	//This is a representation of part 2 of the assignment but also displays part 4
	//PART 4, is Singleton class that controls a global variable which corresponds to the current player in the viewport of the observer design patterns.
	else if (phaseDecision == 4)
	{
		//Observer Design 2, displays information of each player location, view of each territory owned by player, player resources, elektro, houses...
		//Example player pool used  for demo.
		Player p1, p2, p3;
		vector<Player> p;
		p.push_back(p1);p.push_back(p2);p.push_back(p3);
		...
		PowerGrid.setCityForPlayer(p[0], "Seattle");
		PowerGrid.setCityForPlayer(p[0], "Portland");
		PowerGrid.setCityForPlayer(p[0], "Boise");

		PowerGrid.setCityForPlayer(p[1], "Fargo");
		PowerGrid.setCityForPlayer(p[1], "Duluth");
		PowerGrid.setCityForPlayer(p[1], "Minneapolis");

		PowerGrid.setCityForPlayer(p[2], "Salt Lake City");
		PowerGrid.setCityForPlayer(p[2], "Las Vegas");
		staticZero();
		//Implemented Observer design for view of different statistics
		for (int i = 0; i < p.size(); i++)
		{
